...
app.get('/search', async (c) => {
    ...
    const indexName = c.req.query('index') || 'documents';
...
app.post('/search', async (c) => {
    try {
        ...
        const { query = '', index: indexName = 'documents', limit = 20, offset = 0, filter, sort, attributesToRetrieve, attributesToHighlight } = body;
        const index = meiliClient.index(indexName);
        const results = await index.search(query, {
            limit,
            offset,
